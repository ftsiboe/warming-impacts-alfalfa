...
To reproduce results from the shipped `data/` and `output/`, start at `003`
(`003` → `005` → `006` → `100`), then build the article (below). Scripts `001`,
`002`, and `004` rebuild the processed panels from **external raw source archives**
(NASS Quick Stats, PRISM, gSSURGO) on the author's machine and are not required
when the processed `data/` is present.
